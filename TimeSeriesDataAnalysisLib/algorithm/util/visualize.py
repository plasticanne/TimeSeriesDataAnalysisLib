...
info=subprocess.run(["conda","info", "--json"], stdout=subprocess.PIPE, shell=True).stdout
#info="\n".join(os.system("conda info --json").split("\n")[1:])

conda_env=json.loads( info)["active_prefix"]

# Schema to use for flatbuffers
_SCHEMA = os.path.join(conda_env,'Lib','site-packages',"TimeSeriesDataAnalysisLib/algorithm/util/schema/schema1.15.fbs").replace("\\","/")
_SCHEMA_dev = resource_loader.get_path_to_datafile("..\\..\\..\\TimeSeriesDataAnalysisLib\\algorithm\\util\\schema\\schema1.15.fbs")
# Where the binary will be once built in for the flatc converter
_BINARY = os.path.join(conda_env,"Library","bin","flatc.exe").replace("\\","/")
...
def CreateHtmlFile(tflite_input, html_output):
  ...
  if tflite_input.endswith(".tflite") or tflite_input.endswith(".bin"):
    ...
    output_path=os.path.sep.join( os.path.split(tflite_input)[:-1])
    
    # Run convert
    cmd = (
        _BINARY + " -t "
        "--strict-json --defaults-json -o {output} {schema} -- {input}  ".format(
            input=tflite_input,output=output_path, schema=_SCHEMA,))
    print(cmd)
    os.system(cmd)
    """real_output = ( os.path.splitext(
        os.path.split(tflite_input)[-1])[0] + ".json")
    print('real_output',real_output)"""
    real_output = ( os.path.splitext(tflite_input)[0] + ".json")
...
def do_visualize(argv):
  try:
    tflite_input = argv[1].replace('/','\\')
    html_output = argv[2].replace('/','\\')
  except IndexError:
